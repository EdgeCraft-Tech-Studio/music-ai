...
class FileIndexManager:
    ...
    def start_real_time_monitoring(self, callback: Optional[Callable] = None):
        ...
        if self.is_monitoring:
            warning("⚠️ Real-time monitoring already active")
            return
        ...
        try:
            ...
            def handle_file_event(event: FileEvent):
                self._handle_real_time_event(event)
                if callback:
                    callback(event)
            
            # Start monitoring
            self.file_watcher.start(self.indexed_folders, handle_file_event)
            self.is_monitoring = True
            
            info("✅ Real-time file monitoring started")
    ...
    def stop_real_time_monitoring(self):
        ...
        try:
            ...
            self.is_monitoring = False
            info("🛑 Real-time file monitoring stopped")
            
        except Exception as e:
            error(f"❌ Error stopping real-time monitoring: {e}")
