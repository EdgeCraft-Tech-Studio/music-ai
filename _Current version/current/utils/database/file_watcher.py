...
class FileEvent:
    ...
    def __init__(self, event_type: str, path: str, dest_path: Optional[str] = None, 
                 timestamp: float = None, event_id: Optional[str] = None):
...
def init_event_tracking_db(db_path: str):
    ...
    cursor.execute('CREATE INDEX IF NOT EXISTS idx_event_log_processed ON event_log(processed)')
...
class MacFileWatcher(FileWatcher):
    ...
    def start(self, paths: Set[str], callback) -> None:
        ...
        try:
            ...
            error("❌ fsevents library not available. Install with: pip install fsevents")
            raise
    ...
    def reconcile(self, paths: Set[str], callback) -> Dict:
        ...
        try:
            ...
            if not usn_checkpoint:
                ...
                return {'events_processed': 0, 'method': 'usn_journal', 'checkpoint': None}
    ...
    def start(self, paths: Set[str], callback) -> None:
        try:
            ...
            for hDir, base_path, cb in self.handles:
                t = threading.Thread(target=self._watch_directory, args=(hDir, base_path, cb), daemon=True)
                t.start()
    ...
    def _watch_directory(self, hDir, base_path, callback):
        ...
        while self.is_running:
            try:
                ...
                results = win32file.ReadDirectoryChangesW(hDir, 8192, True, notify, None, None)
                for action, name in results:
                    ...
                    if kind in ("created", "modified", "moved_new"):
                        ...
                        evt = FileEvent("created" if kind == "moved_new" else kind, full)
                        callback(evt)
                    elif kind == "deleted":
...
class LegacyFileWatcher(FileWatcher):
    ...
    def start(self, paths: Set[str], callback) -> None:
        ...
        try:
            ...
            class LegacyHandler(FileSystemEventHandler):
                ...
                def _should_ignore_file(self, file_path: str) -> bool:
                    ...
                    try:
                        ...
                        if filename.endswith(('.db', '.db-journal', '.db-wal', '.db-shm')):
                            return True
                ...
                def on_moved(self, event):
                    if not event.is_directory:
                        ...
                        if (not self._should_ignore_file(event.src_path) and 
                            not self._should_ignore_file(event.dest_path)):
                            file_event = FileEvent('moved', event.src_path, event.dest_path)
                            self.callback(file_event)
            ...
            error("❌ watchdog library not available. Install with: pip install watchdog")
            raise
        except Exception as e:
    ...
    def reconcile(self, paths: Set[str], callback) -> Dict:
        ...
        try:
            ...
            events_processed = (sync_results.get('files_added', 0) + 
                              sync_results.get('files_removed', 0) + 
                              sync_results.get('files_updated', 0))
            
            info(f"✅ Legacy reconciliation completed: {events_processed} changes processed")
            return {
                'events_processed': events_processed,
...
class PatchIOFileHandler(FileSystemEventHandler):
    ...
    def __init__(self, db_path: str, indexed_folders: Set[str]):
        ...
        info("🔍 File watcher initialized with {} indexed folders".format(len(indexed_folders)))
    ...
    def _should_ignore_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            # Skip database files (we don't want to index our own database)
            if filename.endswith(('.db', '.db-journal', '.db-wal', '.db-shm', '.sqlite', '.sqlite3')):
                return True
    ...
    def _is_supported_file(self, file_path: str) -> bool:
        ...
        info(f"🔍 File extension check: {ext} -> {'✅ supported' if is_supported else '❌ not supported'}")
        return is_supported
    ...
    def _is_in_indexed_folder(self, file_path: str) -> bool:
        ...
        try:
            ...
            for folder in self.indexed_folders:
                ...
                if folder_abs.startswith('/Volumes/') and abs_path.startswith('/Volumes/'):
                    folder_volume = folder_abs.split('/')[2]  # Get volume name
                    file_volume = abs_path.split('/')[2]      # Get volume name
                    if folder_volume == file_volume:
                        info(f"✅ File is on same volume as indexed folder: {folder_volume}")
                        return True
    ...
    def _add_to_batch(self, operation: str, file_path: str, **kwargs):
        ...
        with self.lock:
            ...
            info(f"📝 Added to batch: {operation} - {file_path} (queue size: {len(self.pending_operations)})")
            
            # Process batch if it's time
            if (len(self.pending_operations) >= self.batch_size or 
                time.time() - self.last_batch_time >= self.batch_delay):
                info(f"⏰ Processing batch (size: {len(self.pending_operations)}, delay: {time.time() - self.last_batch_time:.1f}s)")
                self._process_batch()
    ...
    def _process_batch(self):
        ...
        try:
            ...
            for op in operations:
                try:
                    info(f"🔄 Processing operation: {op['operation']} - {op['file_path']}")
                    if op['operation'] == 'add':
                    ...
                    error("❌ Error processing operation {}: {}".format(op['operation'], e))
                    import traceback
                    traceback.print_exc()
    ...
    def _add_file_to_db(self, cursor, file_path: str):
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (file_path, file_name, vendor, library, project, file_type, stat.st_mtime))
    ...
    def _remove_file_from_db(self, cursor, file_path: str):
        ...
        try:
            ...
            if existing_file:
                ...
                if deleted_count > 0:
                    info("➖ Successfully removed file: {} (ID: {})".format(file_name, file_id))
                else:
        ...
        try:
            ...
            if '.Trashes' in new_path or 'Trash' in new_path:
                ...
                if cursor.rowcount > 0:
                    info("➖ Removed file (moved to Trash): {}".format(os.path.basename(old_path)))
                else:
            ...
            if '.Trashes' in old_path or 'Trash' in old_path:
                ...
                if self._is_supported_file(new_path) and self._is_in_indexed_folder(new_path):
                    self._add_file_to_db(cursor, new_path)
                    info("✅ Restored file from Trash: {}".format(os.path.basename(new_path)))
                else:
                    info(f"⚠️ Restored file not supported or not in indexed folder: {new_path}")
                return
            ...
            if not self._is_in_indexed_folder(new_path):
                ...
                cursor.execute("DELETE FROM files WHERE path = ?", (old_path,))
                if cursor.rowcount > 0:
                    info("➖ Moved file outside indexed area: {}".format(os.path.basename(old_path)))
                else:
                    info(f"ℹ️ File not found in database: {old_path}")
                return
            ...
            new_vendor, new_library, new_project = self._extract_vendor_library_from_path(new_path)
            ...
            if cursor.rowcount > 0:
                info("🔄 Moved file: {} -> {}".format(os.path.basename(old_path), new_name))
            else:
                info(f"ℹ️ File not found in database: {old_path}")
    ...
    def on_moved(self, event):
        if not event.is_directory:
            ...
            if (not self._should_ignore_file(event.src_path) and 
                ...
                self._process_file_immediately('move', event.src_path, dest_path=event.dest_path)
...
class PatchIOFileWatcher:
    ...
    def _batch_processor(self):
        ...
        while self.is_running:
            ...
            if (self.handler.pending_operations and 
                time.time() - self.handler.last_batch_time >= self.handler.batch_delay):
                self.handler._process_batch()
