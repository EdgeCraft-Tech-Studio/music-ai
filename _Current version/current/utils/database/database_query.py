...
class DatabaseQuery:
    ...
    def search_files(
        ...
        try:
            ...
            if query:
                conditions.append(
                    "(name LIKE ? OR path LIKE ? OR vendor LIKE ? OR library LIKE ? OR instrument LIKE ? OR genre LIKE ? OR mood LIKE ? OR format LIKE ?)"
                )
                params.extend(
                    [
                        f"%{query}%",
...
def demo_queries():
    ...
    try:
        ...
        print(
            "❌ Database not found. Please run the indexer first to create the database."
        )
    except Exception as e:
