...
class AutomaticTagger:
    ...
    def _populate_default_mappings(self):
        ...
        try:
            ...
            # Insert all mappings
            all_mappings = instrument_mappings + genre_mappings + mood_mappings + format_mappings
    ...
    def _load_tag_mappings(self):
        ...
        try:
            ...
            self.cursor.execute('SELECT pattern, instrument, genre, mood, format, confidence, is_regex FROM tag_mappings')
            mappings = self.cursor.fetchall()
            
            self.tag_mappings = defaultdict(list)
            for pattern, instrument, genre, mood, format, confidence, is_regex in mappings:
    ...
    def _extract_tags_rule_based(self, text: str) -> TagResult:
        ...
        for tag_type in ['instrument', 'genre', 'mood', 'format']:
            ...
            for mapping in self.tag_mappings[tag_type]:
                ...
                if mapping['is_regex']:
                    ...
                    if pattern.lower() in text_lower:
                        ...
                        pattern_details.append((pattern, adjusted_confidence, len(pattern)))
            ...
            if matches:
                ...
                sorted_matches = sorted(zip(matches, confidences, pattern_details), 
                                      key=lambda x: (x[2][2], x[1]), reverse=True)
                ...
                if tag_type == 'instrument':
                    ...
                    instrument_families = {
                        'woodwind': ['woodwind', 'flute', 'oboe', 'clarinet', 'bassoon', 'saxophone', 'crumhorn', 'shawm', 'recorder', 'bagpipe'],
                        'brass': ['brass', 'trumpet', 'trombone', 'horn', 'tuba', 'french horn'],
                        'strings': ['strings', 'violin', 'viola', 'cello', 'bass', 'guitar', 'harp'],
                        'percussion': ['percussion', 'drums', 'drum', 'snare', 'kick', 'hihat', 'cymbal'],
                        'synthesizer': ['synthesizer', 'synth', 'pad', 'lead', 'bass'],
                        'piano': ['piano'],
                        'fx': ['fx', 'effects', 'stabs', 'hits', 'impacts', 'risers', 'sweeps', 'transitions']
                    }
                    ...
                    for match, confidence, pattern_info in sorted_matches:
                        ...
                        if match_family:
                            ...
                            if match_family not in family_best_matches or length > family_best_matches[match_family][2][2]:
                                family_best_matches[match_family] = (match, confidence, pattern_info)
                    ...
                    for match, confidence, pattern_info in family_best_matches.values():
                        if match not in seen and confidence > 0.3:  # Filter out very low confidence matches
                ...
                total_confidence += max(confidences)  # Use highest confidence for this tag type
                match_count += 1
    ...
    def tag_file(self, file_path: str, library_name: str = "", vendor_name: str = "") -> TagResult:
        """
        ...
        try:
            ...
            # Calculate overall confidence
            confidences = [r.confidence for r in [library_result, filename_result, path_result] if r.confidence > 0]
            if confidences:
    ...
    def add_tag_mapping(self, pattern: str, tag_type: str, tag_value: str, 
                       confidence: float = 1.0, is_regex: bool = False) -> bool:
...
def tag_file(file_path: str, library_name: str = "", vendor_name: str = "", 
             tag_db_path: str = "patchio_knowledge.db") -> TagResult:
