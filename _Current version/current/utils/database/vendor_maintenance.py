...
class AIVendorFinder:
    ...
    def __init__(
        ...
        if api_key:
            ...
            if cached_api_key:
                ...
                raise ValueError(
                    "OpenAI API key not provided. Set OPENAI_API_KEY environment variable, pass api_key parameter, or ensure Google Drive access."
                )
    ...
    def get_sample_paths_for_library(
    ...
    def group_libraries_by_similarity(
        self, libraries: List[Tuple[str, int]], batch_size: int = 10
    ...
    def ask_chatgpt_for_vendors_batch(
        self, library_batch: List[Tuple[str, int]]
    ) -> Dict[str, VendorLibraryInfo]:
...
"""
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a music production expert specializing in sample libraries and music software vendors.",
                    },
    ...
    def update_database(self, vendor_info: VendorLibraryInfo) -> bool:
        ...
        try:
            ...
            self.cursor.execute(
                update_query, (vendor_info.vendor_name, vendor_info.library_name)
            )
            ...
            print(
                f"✅ Updated {updated_count} files: {vendor_info.library_name} → {vendor_info.vendor_name}"
            )
            return True
    ...
    def process_libraries_optimized(
        ...
        print(
            f"📊 Processing {len(unknown_libraries)} unknown libraries in batches of {batch_size}"
        )
        ...
        library_groups = self.group_libraries_by_similarity(
            unknown_libraries, batch_size
        )

        print(f"\n🚀 Starting optimized AI vendor identification...")
        print(
            f"📦 Processing {len(library_groups)} batches (vs {len(unknown_libraries)} individual calls)"
        )
        print("=" * 60)
        ...
        for batch_num, library_batch in enumerate(library_groups, 1):
            ...
            for library_name, file_count in library_batch:
                if library_name in vendor_infos:
                    ...
                    print(
                        f"🎯 {library_name} → {vendor_info.vendor_name} (confidence: {vendor_info.confidence:.2f})"
                    )
    ...
    def print_statistics(self):
        ...
        print(
            f"Success rate: {(self.stats['successful'] / max(self.stats['processed'], 1)) * 100:.1f}%"
        )
        print(f"API calls made: {self.stats['api_calls_made']}")
        print(f"Estimated cost: ${self.stats['estimated_cost']:.3f}")
        print(
            f"Cost per library: ${self.stats['estimated_cost'] / max(self.stats['processed'], 1):.4f}"
        )
...
class KnowledgeToIndexSync:
    ...
    def __init__(self, knowledge_db_path: str, index_db_path: str):
        ...
        # Statistics
        self.stats = {"total_mappings": 0, "updated_files": 0, "libraries_processed": 0}
    ...
    def sync_knowledge_to_index(self, dry_run: bool = False) -> Dict:
        ...
        for vendor, library in vendor_library_mappings:
            ...
            if not dry_run:
                ...
                print(
                    f"🧪 [DRY RUN] Would update {would_update} files: {library} → {vendor}"
                )
...
class VendorMaintenanceWorkflow:
    ...
    def run_complete_workflow(self, dry_run: bool = False) -> Dict:
        ...
        print(
            f"  - Libraries with unknown vendors: {self.stats['unknown_libraries_before']}"
        )
    ...
    def print_final_report(self):
        ...
        print(
            f"  - Improvement: {self.stats['unknown_libraries_before'] - self.stats['unknown_libraries_after']} libraries identified"
        )
        ...
        success_rate = (
            self.stats["ai_identifications"]
            / max(self.stats["unknown_libraries_before"], 1)
        ) * 100
...
def main():
    ...
    parser.add_argument(
        "--batch-size", type=int, default=10, help="Batch size for AI processing"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    try:
        if args.command == "ai-find":
            ...
            stats = finder.process_libraries_optimized(
