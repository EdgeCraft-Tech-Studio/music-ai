...
class MVCPatchIOLoader:
    ...
    def load_ui_file(self):
        ...
        if not self.ui_file:
            ...
            error("❌ Please check that settings/core_settings.py exists and contains UI_FILE_PATH")
            raise ValueError("UI file path is None - settings may not be loaded correctly")
        ...
        if not ui_file_path.exists():
            ...
            warning(f"🔍 Expected path: {os.path.join(os.path.dirname(__file__), self.ui_file)}")
    ...
    def connect_ui_elements(self, ui_widget):
                ...
                if layout:
                    ...
                    for i in range(layout.count()):
                        ...
                        if item.widget() == original_search_input:
                            ...
                            bubble_search_input.setPlaceholderText(original_search_input.placeholderText())
                            bubble_search_input.setStyleSheet(original_search_input.styleSheet())
                            bubble_search_input.setAlignment(original_search_input.alignment())
                            bubble_search_input.setMinimumSize(original_search_input.minimumSize())
                            bubble_search_input.setMaximumSize(original_search_input.maximumSize())
                            bubble_search_input.setSizePolicy(original_search_input.sizePolicy())
                            
                            # Replace the original widget in the layout
                            layout.replaceWidget(original_search_input, bubble_search_input)
        ...
        main_window.start_search_button = ui_widget.findChild(QWidget, "startSearchButton")
        ...
        main_window.keywords_editor_button = ui_widget.findChild(QWidget, "advancedButton")
    ...
    def setup_filter_button_icons(self):
        ...
        try:
            ...
            debug(f"🔍 Found buttons: instruments={instruments_button}, genres={genres_button}, vendor={vendor_button}")
    ...
    def setup_filter_button_fallback_arrows(self):
        ...
        try:
            ...
            debug(f"🔍 Found buttons for fallback: instruments={instruments_button}, genres={genres_button}, vendor={vendor_button}")
        ...
        try:
            ...
            if ai_button and hasattr(ai_button, 'setIcon'):
                ...
                if not ai_button.icon():
                    # Load from file as fallback
                    from pathlib import Path
                    icon_filename = ICON_PATHS.get('ai_search_icon', 'ai_search_icon.png')
                    icon_path = Path(__file__).parent / "assets/icons" / icon_filename
                    # Resolve the path to handle relative paths properly
                    icon_path = icon_path.resolve()
                    if icon_path.exists():
    ...
    def setup_classic_search_button_icon(self):
        ...
        try:
            classic_search_button = self.main_controller.main_window.classic_search_button
                ...
                if not classic_search_button.icon():
                    ...
                    if icon_path.exists():
                        ...
                        if not icon.isNull():
                            ...
                            warning("⚠️ Could not load Classic Search button icon from file")
                    else:
                        warning("⚠️ Classic Search button icon file not found: {}".format(icon_path))
                else:
                    debug("✅ Classic Search button icon already loaded from .ui file")
            else:
                warning("⚠️ Classic Search button widget not found or does not have setIcon")
    ...
    def setup_keywords_editor_button_icon(self):
        ...
        try:
            keywords_editor_button = self.main_controller.main_window.keywords_editor_button
            if keywords_editor_button and hasattr(keywords_editor_button, 'setIcon'):
                # Try to load from .ui file first (if already set)
                if not keywords_editor_button.icon():
                    # Load from file as fallback
                    from pathlib import Path
                    icon_filename = ICON_PATHS.get('keywords_editor', 'advanced_mode.png')
                    icon_path = Path(__file__).parent / "assets/icons" / icon_filename
                    # Resolve the path to handle relative paths properly
                    icon_path = icon_path.resolve()
                    if icon_path.exists():
                        ...
                        if not icon.isNull():
                            ...
                            warning("⚠️ Could not load Keywords Editor button icon from file")
                    else:
                        warning("⚠️ Keywords Editor button icon file not found: {}".format(icon_path))
                else:
                    debug("✅ Keywords Editor button icon already loaded from .ui file")
            else:
                warning("⚠️ Keywords Editor button widget not found or does not have setIcon")
