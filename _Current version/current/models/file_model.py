...
class FileModel:
    ...
    def extract_musical_metadata(self, file_name: str, file_path: str) -> Dict[str, str]:
    ...
    def extract_library_root_patchio_style(self, path: str) -> str:
        ...
        for i, part in enumerate(parts):
            ...
            if i + 1 < len(parts):
                ...
                content_indicators = [
                    'instruments', 'samples', 'presets', 'patches', 'articulations',
                    'data', 'layers', 'keyswitches', 'resources', 'audio', 'sounds', 'loops',
                    'multis', 'banks', 'patches', 'samples', 'instruments'
                ]
                ...
                if any(word in next_part for word in ['multis', 'banks', 'patches', 'samples']):
                    score += 15
                    
            # Check if this folder name looks like a product name (not just a generic folder)
            if len(part) > 3 and not part_lower in ['content', 'files', 'data', 'library']:
                score += 10
            ...
            if ' - ' in part or (len(part.split()) >= 2 and any(word in part.lower() for word in ['library', 'collection', 'bundle', 'pack'])):
            ...
            if part_lower in ['instruments', 'samples', 'presets', 'patches', 'articulations']:
    ...
    def _detect_collection_pattern(self, parts):
        ...
        for i, part in enumerate(parts):
            ...
            if len(part.split()) == 1 and part.lower() in ['up', 'down', 'left', 'right', 'close', 'far', 'wide', 'spot', 'mix', 'decca']:
                continue
            ...
            try:
                for filename in os.listdir(current_dir):
                    if filename.endswith(('.logic', '.logicx', '.cpr')):
                        ...
                        return os.path.basename(current_dir)  # Stop immediately when found
            except (OSError, PermissionError):
    ...
    def get_library_key_patchio_style(self, path: str) -> str:
        ...
        generic_dirs = {
            ...
            "loops", "sessions", "projects", "tracks", "stems", "mixdown"
        }
        ...
        for i in range(len(parts) - 1, 1, -1):
            ...
            if part_lower in ['libraries', 'kontakt libraries', 'player libraries', 'non player libraries']:
                continue
    ...
    def group_results_by_library(self, results: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
