...
class SearchController:
    ...
    def __init__(self, search_model: SearchModel, file_model: FileModel, main_window: MainWindowView):
        ...
        self.polling_timer.start(100)  # Poll every 100ms instead of 25ms for better performance
    ...
    def on_search_triggered(self):
        ...
        main_controller_filters = self.main_controller.get_active_filters() if hasattr(self, 'main_controller') else {}
    ...
    def build_search_query(self, search_text: str = None, active_filters: dict = None) -> str:
        ...
        if active_filters is None:
            if hasattr(self, 'main_controller') and hasattr(self.main_controller, 'get_active_filters'):
    ...
    def on_mode_changed(self, mode: str):
        ...
        try:
            ...
            # Update UI elements through the main window
            if self.main_window:
                self.main_window.update_mode_buttons()
                self.main_window.update_search_tooltip()
                # Switch search input based on mode
                self.main_window.switch_search_input(mode)
            
            # Don't re-run search - just change the mode for future searches
            # The current results should stay visible
    ...
    def on_search_cleared(self):
        ...
        self.main_window.clear_results()
        self.main_window.update_status_display("Ready to search your sound files", "ready")
    ...
    def _perform_simple_search_dynamic(self, query: str, search_id: int):
        ...
        try:
            ...
            or_terms, and_terms, not_terms, or_quoted, and_quoted, not_quoted = self.search_model._parse_bubble_query(simple_raw)
        ...
        for folder in self.search_model.search_folders:
            ...
            try:
                ...
                for file_path in self.search_model._recursive_scan(str(folder), self.search_model.selected_extensions, excluded_folders):
                    ...
                    or_match = (not or_terms) or any(
                        self.search_model._matches_term(term, full_path, quoted) if term else True 
                    ...
                    not_match = (not not_terms) or not any(self.search_model._matches_term(term, full_path, quoted) for term, quoted in zip(not_terms, not_quoted))
                    ...
                    if or_match and and_match and not_match:
                        ...
                        # Add search metadata
                        keywords = self.search_model.get_matched_keywords(file_path, query)
    ...
    def poll_search_results(self):
        ...
        try:
            ...
            if self.should_stop_search:
                debug("🛑 Search cancelled - clearing pending results and stopping processing")
                self.pending_results = []
                return
            ...
            while not self.search_queue.empty() and results_processed < self.batch_size:
                ...
                if result_search_id != self.current_search_id:
                    warning(f"🛑 Ignoring result from old search {result_search_id} (current: {self.current_search_id})")
                    continue
                ...
                if result_type == 'clear':
                    ...
                    self.main_window.clear_results()
                    self.main_window.show_status_message(f"{SEARCH_RESULTS_LABEL} {result_data.get('query', '')}")
                    ...
                    # Search completed
                    total_files = sum(lib_data['count'] for lib_data in self.library_nodes.values())
                    total_libraries = len(self.library_nodes)
                    self.main_window.complete_search_progress(total_files, total_libraries)
                    
                elif result_type == 'error':
                    # Search error
                    error_msg = result_data.get('error', 'Unknown error')
                    self.main_window.update_status_display(f"❌ Search error: {error_msg}", "error")
            ...
            if not self.should_stop_search and (len(self.pending_results) >= self.batch_size or (self.pending_results and results_processed == 0)):
                self.process_pending_results()
    ...
    def process_pending_results(self):
        ...
        try:
            ...
            debug(f"✅ Processed batch: {total_files} files in {total_libraries} libraries")
    ...
    def add_single_result_optimized(self, result: Dict[str, Any]):
        ...
        try:
            ...
            if library_name not in self.library_nodes:
                ...
                self.main_window.results_tree.setItemWidget(library_item, 1, file_type_widget)
                ...
                if result.get('keywords'):
                    self.library_nodes[library_name]['keywords'].update(result['keywords'].split(' • '))
                if result.get('tags'):
                    self.library_nodes[library_name]['tags'].update(result['tags'].split(' • '))
                ...
                library_item.setText(0, f"📁 {library_name} ({library_data['count']} files)")
    ...
    def adjust_performance_for_large_results(self):
        ...
        if self.total_results_processed >= self.performance_adjustment_threshold:
            ...
            if self.batch_size < 50:
                ...
                debug(f"🔧 Increased batch size to {self.batch_size} for large result set")
            
            if self.update_throttle_ms < 200:
                self.update_throttle_ms = min(200, self.update_throttle_ms + 25)
                debug(f"🔧 Increased update throttle to {self.update_throttle_ms}ms for large result set")
    ...
    def populate_results_tree(self, results: Dict[str, Dict[str, Any]]):
        ...
        try:
            ...
            self.main_window.results_tree.setHeaderLabels(["File Name", "File Type", "Tags"])
            ...
            if self.main_window.results_tree:
                ...
                if current_width < min_file_type_width:
                    ...
                    debug(f"🔧 Set File Type column width to minimum: {min_file_type_width}px")
            ...
            header.setSectionResizeMode(1, QHeaderView.ResizeToContents)  # File Type - auto-resize to content
            ...
            self.main_window.results_tree.setHorizontalScrollMode(QTreeWidget.ScrollPerPixel)
            ...
            for library_name, library_data in results.items():
                ...
                file_type_str = ', '.join(sorted(file_types)) if file_types else "Library"
                ...
                for file_data in library_data['files']:
                    ...
                    # File type
                    file_type = self.file_model.get_file_type_info(file_data.get('path', ''))
                    file_type_widget = ResultsTreeBubbleWidget(file_type)
                    self.main_window.results_tree.setItemWidget(file_item, 1, file_type_widget)
                    ...
                    self.main_window.results_tree.setItemWidget(file_item, 2, keywords_widget)
            ...
            if self.main_window.results_tree:
                ...
                if current_width < min_file_type_width:
                    self.main_window.results_tree.setColumnWidth(1, min_file_type_width)
                    debug(f"🔧 Final check: Set File Type column width to {min_file_type_width}px")
    ...
    def calculate_optimal_column_widths(self, results: Dict[str, Dict[str, Any]]):
        ...
        self.main_window.results_tree.setColumnWidth(0, max(min_file_name_width, min(max_file_name_width + file_name_padding, max_file_name_width_setting)))
        self.main_window.results_tree.setColumnWidth(1, max(min_file_type_width, min(max_file_type_width + file_type_padding, max_file_type_width_setting)))
        self.main_window.results_tree.setColumnWidth(2, max(min_keywords_width, min(max_keywords_width + keywords_padding, max_keywords_width_setting)))
        self.main_window.results_tree.setColumnWidth(3, max(min_tags_width, min(max_tags_width + tags_padding, max_tags_width_setting)))
    ...
    def setup_double_click_handler(self):
        """Setup double-click handler for opening parent folder"""
        if self.main_window.results_tree:
            self.main_window.results_tree.itemDoubleClicked.connect(self.on_item_double_clicked)
